...
class AutosActivity : AppCompatActivity() {
    ...
    private fun loadFacturas(): List<String> {
        return gestorSQL.getAuto().map { it.detalle + " - " + it.ubicacion }
    }
    ...
    override fun onCreateContextMenu(menu: ContextMenu, v: View, menuInfo: ContextMenu.ContextMenuInfo?) {
        ...
        menuInflater.inflate(R.menu.auto_context_menu, menu)
    }

    override fun onContextItemSelected(item: MenuItem): Boolean {
        val info = item.menuInfo as AdapterView.AdapterContextMenuInfo
        when (item.itemId) {
            R.id.edit -> editAutos(info.position)
            R.id.delete -> {
                gestorSQL.deleteAuto(gestorSQL.getAuto()[info.position].id)
                updateListView()
            }
            R.id.map -> showMap(gestorSQL.getAuto()[info.position].ubicacion)
            else -> return super.onContextItemSelected(item)
        }
        return true
    }

    private fun editAutos(position: Int) {
        val factura = gestorSQL.getAuto()[position]
        ...
        builder.setPositiveButton("Guardar") { dialog, which ->
            ...
            if (parts.size >= 2) {
                gestorSQL.updateAuto(factura.id, parts[0], parts[1])
                updateListView()
            }
        }
